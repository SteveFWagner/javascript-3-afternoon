...
//Code Here
// let myStrongest = monstersInYourPocket.filter((e,i,arr)=>arr[e].cp > 200?arr[e]:arr.splice(i,1))
let myStrongest = monstersInYourPocket.filter((val, i, arr)=>{
  if(val.CP>200){return val}
})
...
orders.forEach((val,i,arr)=> {
  val = val.price+val.tax
  orderTotals.push(val)
  })
